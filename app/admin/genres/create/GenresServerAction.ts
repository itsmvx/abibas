...
export const genresCreateData = async (formData: FormData): Promise<{ success: boolean, message: string }> => {
...
export async function genresNameCheck(name: string): Promise<boolean | null> {
    const { key, iv }: { key: string | null, iv: string | null } = getApiKey();
    if (!key || !iv) {
        return null;
    }

    const res: Response = await fetch(`${ process.env.MY_WEBAPI_URL }/admin/genres?check=${ name.toLowerCase() }`, {
        method: 'GET',
        headers: {
